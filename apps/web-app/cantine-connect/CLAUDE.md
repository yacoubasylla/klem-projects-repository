...
5.  **Manuel Utilisateur — Mise à jour obligatoire à chaque livraison de fonctionnalité :** Depuis la réorganisation documentaire du 2026-08-04, les livrables client (`manuel-utilisateur.md`/`.docx`/`.pdf`, `cahier-de-recette.md`/`.docx`, `offre-financiere-cantine-connect.md`/`.pdf`, captures d'écran) ne vivent plus dans ce dépôt : ils sont édités dans `klem-labs-repository/projects/03_cantine_connect/livraison/`, qui en est la source de vérité (voir `klem-labs-repository/README.md` §1). Ce dépôt ne garde qu'une copie en lecture seule sous `docs/from-labs/livraison/`. Les trois fichiers `manuel-utilisateur.*` partagent volontairement le même nom de base (pas de nom alternatif type `user-guide` — source .md et livrables .docx/.pdf identifiés d'un coup d'œil). Avant de clore toute tâche qui change un comportement visible par l'utilisateur (nouveau module, nouvelle règle métier, nouveau champ/filtre, changement de permission par rôle) :
    ...
    *   Régénérer `manuel-utilisateur.docx`/`.pdf` depuis le `.md` mis à jour. Sans `pandoc` disponible : convertir le `.md` en HTML (`npx marked manuel-utilisateur.md -o /tmp/corps.html`), l'insérer dans un gabarit HTML avec styles + `@page { size: 21cm 29.7cm; margin: 2cm; }` (le mot-clé CSS `A4` seul est ignoré par le filtre d'import de LibreOffice — utiliser les dimensions explicites). Écueils rencontrés, tous vérifiés en pratique (LibreOffice headless v7.3.7.2) :
        - **Redimensionnement des images** : ni `width`/`max-width` en `pt` ou `%` (feuille `<style>` **ou** inline), ni `height` explicite, ne sont fiables sur `<img>` — au mieux ignorés (l'image garde sa taille intrinsèque, ~96dpi, largement plus large qu'une page A4), au pire un couple `width`+`height` explicite fait *cropper* l'image au lieu de la redimensionner. La seule méthode fiable : **redimensionner les fichiers PNG sources eux-mêmes** avant l'injection dans le HTML (ex. Python/Pillow, largeur cible ≈ 600px pour un affichage à ~450pt) puis ne poser **aucun** style de taille sur `<img>` — seulement bordure/radius/margin, qui eux passent bien par la feuille `<style>` globale.
        - **Styles de tableau** (bordures, couleur d'en-tête, alternance de lignes) : **jamais** honorés via une feuille `<style>` globale, pour le PDF **comme** pour le DOCX (une affirmation contraire a précédemment traîné dans cette note — vérifiée fausse) — vérifier en rendant une page en image (`pdftoppm`) plutôt qu'en supposant que la CSS s'est appliquée. Toujours injecter le style **inline** sur `<table>`/`<th>`/`<td>` (le package `cheerio` aide à le faire proprement) : `border` en unité `px` (`pt` est silencieusement ignoré sur `th`/`td` par ce moteur) et `background-color` (pas le raccourci `background`, ignoré par `html-to-docx`).
        - **Emoji colorés** (✅❌🎨🍽️📞📧, etc.) : silencieusement invisibles (absents même du texte extrait, pas juste un glyphe manquant) — cet environnement n'a pas de police d'emoji couleur. Utiliser des symboles simples correctement supportés (`✓`/`✗` à la place de `✅`/`❌`) ou du texte brut ("Téléphone :" à la place de 📞) — jamais d'emoji décoratif dans le `.md` source.
        - **PDF** : `soffice --headless --convert-to 'pdf:writer_pdf_Export' fichier.html` directement depuis le HTML (repartir du `.docx` réintroduit le format Letter par défaut).
        - **DOCX** : ⚠️ `soffice --headless --convert-to 'docx:MS Word 2007 XML' fichier.html` **ne fonctionne pas dès que le `.md` contient des images** — le filtre d'import HTML de LibreOffice headless ne charge pas les `<img>` locaux (ni `file://`, ni chemin relatif) dans le modèle du document : le `.docx` produit est valide mais silencieusement dépourvu du dossier `word/media/`, à vérifier avec `unzip -l fichier.docx | grep media` avant de considérer la conversion réussie. Utiliser à la place `npx html-to-docx` (package npm, images en data URI base64) : encoder chaque image référencée en base64 et l'injecter en `src="data:image/png;base64,..."`.
        - Script de référence utilisé pour la dernière régénération complète (marked → cheerio → soffice/html-to-docx) : voir l'historique de session, non committé dans ce dépôt (script jetable en dehors du repo).
