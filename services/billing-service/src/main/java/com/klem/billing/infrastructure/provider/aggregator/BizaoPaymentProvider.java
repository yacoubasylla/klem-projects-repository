 ...
 * documentation API Bizao du compte marchand KLEM avant mise en production.
 */
@Component
public class BizaoPaymentProvider implements PaymentProvider {

    private static final String SIGNATURE_HEADER = "x-bizao-signature";

    private final RestClient restClient;
    private final AggregatorProperties.Provider config;

    public BizaoPaymentProvider(RestClient.Builder restClientBuilder, AggregatorProperties aggregatorProperties) {
        this.config = aggregatorProperties.getBizao();
    ...
    public PaymentAggregator aggregator() {
        return PaymentAggregator.BIZAO;
    }
    ...
    public PaymentInitiationResult initiate(PaymentInitiationCommand command) {
        Map<String, Object> requestBody = Map.of(
                ...
                "customerReference", command.payerReference() != null ? command.payerReference() : "anonymous"
        );
        ...
        String redirectUrl = response != null ? (String) response.get("checkoutUrl") : null;
    ...
    public void verifySignature(WebhookCallback callback) {
        ...
        if (!HmacSignatureVerifier.isValid(callback.rawBody(), received, config.getApiSecret())) {
            throw new InvalidWebhookSignatureException("Bizao");
        }
    }
    ...
    public WebhookParseResult parseCallback(WebhookCallback callback) {
        ...
        String idempotencyKey = (String) payload.get("merchantTransactionId");
